import torch.nn as nn
import torch.nn.functional as F

# Define the CNN model with batch normalization and dropout
class ConvNet(nn.Module):
    def __init__(self):
        super(ConvNet, self).__init__()
        self.conv1 = nn.Conv2d(1, 32, 3, padding=1)  # First convolutional layer
        self.bn1 = nn.BatchNorm2d(32)                # Batch normalization layer after conv1
        self.pool1 = nn.MaxPool2d(2, 2)              # First max pooling layer
        
        self.conv2 = nn.Conv2d(32, 64, 3, padding=1) # Second convolutional layer
        self.bn2 = nn.BatchNorm2d(64)                # Batch normalization layer after conv2
        self.pool2 = nn.MaxPool2d(2, 2)              # Second max pooling layer
        
        self.conv3 = nn.Conv2d(64, 128, 3, padding=1) # Third convolutional layer
        self.bn3 = nn.BatchNorm2d(128)                # Batch normalization layer after conv3
        self.pool3 = nn.MaxPool2d(2, 2)               # Third max pooling layer
        
        self.fc1 = nn.Linear(128 * 3 * 3, 256)        # First fully connected layer
        self.drop1 = nn.Dropout(0.5)                  # Dropout layer after fc1
        self.fc2 = nn.Linear(256, 128)                # Second fully connected layer
        self.drop2 = nn.Dropout(0.5)                  # Dropout layer after fc2
        self.fc3 = nn.Linear(128, 10)                 # Output layer

    def forward(self, x):
        x = self.pool1(F.relu(self.bn1(self.conv1(x)))) # Forward pass through first conv, bn, relu, and pool
        x = self.pool2(F.relu(self.bn2(self.conv2(x)))) # Forward pass through second conv, bn, relu, and pool
        x = self.pool3(F.relu(self.bn3(self.conv3(x)))) # Forward pass through third conv, bn, relu, and pool
        x = x.view(-1, 128 * 3 * 3)                     # Flatten the tensor
        x = F.relu(self.fc1(x))                         # Forward pass through first fully connected layer and relu
        x = self.drop1(x)                               # Apply dropout
        x = F.relu(self.fc2(x))                         # Forward pass through second fully connected layer and relu
        x = self.drop2(x)                               # Apply dropout
        x = self.fc3(x)                                 # Output layer
        return x
